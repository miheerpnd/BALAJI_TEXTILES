...
<div class="float-mess">
  <strong class="bg-danger bg-gradient text-light">&nbsp;SELL&nbsp;</strong>
</div>

  <?php require('includes/header.php'); ?>

  <form class="needs-validation" id="MyForm" action="javascript:void(0)" onsubmit="return false;" autocomplete="off" novalidate>
  ...
  <div class="row">
    <div class="col-8 p-4">
      ...
      <div class="row m-auto">
        <div class="col">
          ...
          <input type="text" class="form-control n-i1" name="c_name" id="name_val" placeholder="Name" aria-label="Name" required>
        </div>
        <div class="col">
    ...
    <div class="ui segment raised">
      ...
      <div class="row m-auto">
        ...
        <input type="hidden" name="process_type" id="process_type" value="SELL">
        </div>
      </div>
      <hr>
      ...
      <div class="row m-auto">
        <button class="btn btn-sm bg-gradient btn-dark" id="submitButton">
          SELL & PRINT <strong class="bg-primary text-light" style="font-size: 13px;">&nbsp;Ctrl + P&nbsp;</strong>
        </button>
      </div>
    </div>
    </div>
  ...
  <script>
    ...
    let dev_cart = 0;
    function check_item_data_validation(){
see_validity();
set_validity();
    }

    function see_validity(){
      let text_data = document.querySelector('#cart_item').value;
      let xhttp = new XMLHttpRequest();
  xhttp.onload = function() {
    document.getElementById("replace_sell_button").innerHTML = this.responseText;
    }
  xhttp.open("GET", "php/item_validation/valid.php?val="+text_data);
  xhttp.send();
    }
    function set_validity(){
      let text_data = document.querySelector('#cart_item').value;
      let xhttp = new XMLHttpRequest();
  xhttp.onload = function() {
    document.getElementById("cart_qty").max = this.responseText;
    document.getElementById("stock_show").innerHTML = '- ('+this.responseText+')';
    dev_cart = this.responseText;
    }
  xhttp.open("GET", "php/item_validation/validset.php?val="+text_data);
  xhttp.send();
    }
  </script>
    ...
    <div class="">
      <div class="row">
        <div class="col">
          ...
          <input type="text" id="cart_item" class="form-control n-e1" placeholder="Product" onkeyup="check_item_data_validation()">
        </div>
        <div class="col">
          <label for="">Qty <b id="stock_show"></b></label>
      ...
      <div class="row text-center">
        <div class="col m-auto mt-4" id="replace_sell_button">
        <button class="btn btn-success n-e6" id="btn-add-data" onclick="add_data_carp()" data-bs-dismiss="offcanvas">
          ADD DATA
...
document.addEventListener("keydown", function (e) {
    ...
    document.addEventListener('keydown', function(e){
      if(e.keyCode == 13){
        ...
        // 3 - 4
          ...
          e.preventDefault();
          $('#add_data_to_cart').click();
          setTimeout( function() { focusme(); }, 1000);
        }
        // 4 - 5
  ...
  <script>
    ...
    function add_data_carp(){
      if(document.querySelector('cart_qty').value <= dev_cart){
        add_data_cart();
      }
      else{
